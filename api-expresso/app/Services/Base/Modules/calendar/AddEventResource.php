...
class AddEventResource extends CalendarAdapter {
	...
	public function post( $request ){
		...
		$ownerEvent				= $this->getParam('eventOwnerImport');
		...
		//USADO PARA forï¿½ar A inclusï¿½o DE UM EVENTO IGNORANDO CONFLITOS DA AGENDA.
		$eventIgnoreConflicts               = $this->getParam('eventIgnoreConflicts');

		$this->addModuleTranslation("calendar");

		//VALIDAï¿½ï¿½ES DE CAMPOS 
		...
		//ALARMES E REPETIï¿½ï¿½O DE EVENTOS nï¿½o Sï¿½O UTILIZADAS NA API
		$cal['uid']				= '';
		...
		if ($eventOwnerIsParticipant == "1") {
			$participants[]	= $cal['owner'] . 'A'; // A = ACCEPTED
		}
		...
		//PARAMETRO SENDTOUI ADICIONADO AO BO.CALENDAR PARA EVITAR QUE O BO FAï¿½A O REDIRECIONAMENTO PARA UM TEMPLATE.
		$params['sendToUi'] 		= '0';
