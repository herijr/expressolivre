...
class ContactsResource extends CatalogAdapter {	
	...
	public function post($request){
		...
		if($this->getParams()) {
			...
			$search = ($search ? mb_convert_encoding($search,"ISO-8859-1", "UTF-8") : "");
		...
		while($row = $resQuery->fetchRow()) {
			...
			if($contacts[$id] != null){
				...
				$contacts[$id] = array(
						...
						'contactAlias' => ($row['alias'] != null ?  mb_convert_encoding($row['alias'],"UTF-8", "ISO-8859-1") : ""),
						'contactFirstName'	=> ($row['given_names'] != null ?  mb_convert_encoding($row['given_names'],"UTF-8", "ISO-8859-1") : ""),
						'contactLastName' 	=> ($row['family_names'] != null ?  mb_convert_encoding($row['family_names'],"UTF-8", "ISO-8859-1") : ""),
						'contactFullName' 	=> ($row['names_ordered'] != null ? mb_convert_encoding($row['names_ordered'],"UTF-8", "ISO-8859-1") : ""),
						'contactBirthDate'	=> ($row['birthdate'] != null ? $row['birthdate'] : ""),
						'contactNotes' 		=> ($row['notes'] != null ?  mb_convert_encoding($row['notes'],"UTF-8", "ISO-8859-1") : ""),
