...
function copy_manager(manager)
{
	...
	var handler_copy_manager = function(data)
	{
		if (data.status)
			...
			alert(get_lang(data.msg) + '.');
	}
	//cExecute ('$this.db_functions.copy_manager&manager='+manager+'&new_manager='+new_manager, handler_copy_manager);
	cExecute ('$this.manager.copy_manager&manager='+manager+'&new_manager='+new_manager, handler_copy_manager);
}
...
var searchTimeout;
function search_manager(manager_lid)
{
