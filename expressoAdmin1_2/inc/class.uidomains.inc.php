<?php
	...
	*  option) any later version.														 *
	...
	class uidomains
	{
		...
		function delete()
		{
			$return = $this->delete_domain( $_GET['sambadomainname'] );
			if ( !( isset( $return['status'] ) && $return['status'] === true ) ) {
				echo 'Error: '.$return['msg'];
			}
		...
		function delete_domain( $sambadomainname )
		{
			...
			if ( !( isset( $result_ldap['status'] ) && $result_ldap['status'] === true ) ) return $result_ldap;

			// Remove DB samba domain
			$this->db->delete_sambadomain( $sambadomainname );
