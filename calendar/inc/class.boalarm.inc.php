...
//		var $debug = True;
		...
		*/
		function add(&$event,$time,$owner)
		{
			...
			$alarm = Array(
				'time'    => ($etime=$this->bo->maketime($event['start'])) - ($time - $this->tz_offset),
				'offset'  => $time,
				'owner'   => $owner,
				'enabled' => 1
			);
			...
			$event['alarm'][$alarm['id']]['time'] = $alarm['time'] + $this->tz_offset;	// allows viewing alarm time in users timezone and summertime
			return $alarm;
		}
