<?php
//  Conjunto de funcoes de apoio as Classes de tratamento do certificado digital.

		function pega_path($chaves,$linha)
		{
			if(!is_array($chaves) || !$linha)
				{
					return false;
				}
			$ret = false;
			foreach($chaves as $chave)
				{
					if(substr($linha,0,strlen($chave)) == $chave)
						{
							$c = '';
							if(strpos($linha,'\'') > 0)
								{
	...
	function get_length(&$len,&$bytes,$data)
	{
		$len = ord($data[1]);
		$bytes = 0;
		...
		while (strlen($data) > 1)
		{
			...
			switch ($class)
			{
				...
				default:
					// Unknown
					$result[] = 'UNKNOWN' .  $data;;
					//file_put_contents('/opt/lampp/htdocs/seguranca/temp/arquivo_desconhecido.txt',$data);
					$data = '';
					break;
			}
        }
...
function testa_p7m($msg)
	{
		...
		$p7m_formato_der = base64_decode($ZZ3);
	        $oid_hexa = OIDtoHex('1.2.840.113549.1.7.2');       // converte oid de texto para hexadecimal ...
		$partes = explode($oid_hexa,$p7m_formato_der);    // Faz o split pela oid...
		if(count($partes)>1)
			{
					return 'signature' ;
			}
		$oid_hexa = OIDtoHex('1.2.840.113549.1.7.3');
		$partes = explode($oid_hexa,$p7m_formato_der);    // Faz o split pela oid...
		if(count($partes)>1)
			{
					return 'cipher' ;
...
function parse($oid,$valor)
	{
		//
		//  OID's PESSOA FISICA = 2.16.76.1.3.1 ,  2.16.76.1.3.6 ,  2.16.76.1.3.5 ,  2.16.76.1.4.n ... as 2.16.1.4.n não são obrigatórias e ão sao tratadas..
		//
		//  OID's PESSOA JURIDICA = 2.16.76.1.3.4 ,  2.16.76.1.3.2 , 2.16.76.1.3.3 , 2.16.76.1.3.7
		//
		//  OID's EQUIPAMENTO/APLICAÇÃO = 2.16.76.1.3.8 ,  2.16.76.1.3.3 , 2.16.76.1.3.2 , 2.16.76.1.3.4
		//
...
function subjectAltName($xx, $certificado_digital_formato_der)
	{
		$dados = array();
		$ret = recupera_dados_oid($certificado_digital_formato_der,'2.5.29.17');
		if(count($ret))
		{
...
function AUTHORITYKEYIDENTIFIER($xx, $certificado_digital_formato_der)
	{
		...
		if (isset($certificado_digital_formato_der))
			{
				$caid = recupera_dados_oid($certificado_digital_formato_der,'2.5.29.35');
				$i=1;
...
function KEYUSAGE($xx, $certificado_digital_formato_der)
	{
		...
		if (isset($certificado_digital_formato_der))
			{
				$AUX = recupera_dados_oid($certificado_digital_formato_der,'2.5.29.15');  // busca oid do keyusage
...
function EXTKEYUSAGE($xx, $certificado_digital_formato_der)
	{
		...
		if (isset($certificado_digital_formato_der))
			{
				$AUX = recupera_dados_oid($certificado_digital_formato_der,'2.5.29.37');  // busca oid do extkeyusage
...
function BASICCONSTRAINTS($xx, $certificado_digital_formato_der)
	{
		...
		if (isset($certificado_digital_formato_der))
			{
				$AUX = recupera_dados_oid($certificado_digital_formato_der,'2.5.29.19');  // busca oid do BasicConstraints
				if(count($AUX) > 0)
					{
