...
folderList.prototype.createLine = function(file){
	...
	retBuff += '<span class="fileLink" id="name_'+file.name+'" onclick="window.open(\'./index.php?menuaction=filemanager.uifilemanager.view&file='+base64_encode(file.name)+'&path='+base64_encode(currentPath)+'\');">'+file.name+'</span></div></td>';
	...
	if (preferences.createdby_id =='1')
	{
		...
		retBuff += '<div>'+file.createdby_id+'</div></td>';		
	}
	...
	if (preferences.modifiedby_id =='1')
	{
		...
		retBuff += '<div>'+file.modifiedby_id+'</div></td>';
	}
...
folderList.prototype.updateQuota = function(quotaSize,usedSpace)
{
	...
	{
		contentQuota.innerHTML = '<table width="102" cellspacing="0" cellpadding="0" border="0" id="table_quota">' +
								 '<tbody><tr>' +
								 '<td width="102" nowrap="true" height="15" background="phpgwapi/templates/default/images/dsunused.gif">' +
								 '<table cellspacing="0" cellpadding="0" border="0" style="width: '+parseInt((usedSpace/quotaSize)*100)+'%;">' +
								 '<tbody><tr><td height="15" class="dsused"/></tr></tbody></table>'+
								 '</td><td nowrap="true" align="center"><span class="boxHeaderText">&nbsp;&nbsp;'+
								 parseInt(usedSpace/quotaSize*100)+'% ('+borkb(usedSpace)+'/'+borkb(quotaSize)+')</span>'+
...
function draw_card(type,name)
{
	...
	if( type == "preview" )
	{
		...
		menuTimeout = setTimeout( "draw_window_card(\'"+htmlData+"\')", 300 );
	}
	
	if( type == "user")
	{
		var url = './index.php?menuaction=filemanager.user.card&lid='+base64_encode(name);
		menuTimeout = setTimeout("cExecute_('"+url+"',draw_window_card)", 300 );
	}
...
function editFolders( operation )
{
	...
	if ( operation == 'new' )
	{
		...
		if ( name != null && name != '' )
		{
			...
			var _action = './index.php?menuaction=filemanager.uifilemanager.createdir&path='+ parentDir_en+'&filename='+base64_encode(name);	
			
			cExecute_( _action , handler.refreshDir );
		...
		if ( operation == 'remove' )
		{
			...
			{	
				if ( confirm(get_lang('Do you really want to remove folder: %1?',
					...
					var _action = './index.php?menuaction=filemanager.uifilemanager.removedir&path=' + Dfolder_en; 
					cExecute_( _action , handler.refreshDir );
...
function draw_window_card(content)
{
	...
	if( menuTimeout )
		clearTimeout(menuTimeout);
	menuTimeout = setTimeout("hide_card()",1000);
}
function hide_card()
{
