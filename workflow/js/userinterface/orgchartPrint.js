...
/**
* Show / Hide mail column
*/
function toggleMailVisibility()
{
	$('#employee_table').remove();
...
*/
function createEmployeeRow(area_id, user_id, showAreaColumn, showUserPhoto, showUserMail, showUserCategory)
{
	...
	if (showAreaColumn)
		element.append(
					$('<td valign="top">' + areas[area_id].sigla + '</td>')
						.css('width', '8%')
					);
	...
	/* mail: show mail attribute */
	if (showUserMail)
		element.append(
					$('<td valign="top">' + areas[area_id].employees[user_id].mail + '</td>')
						.css('width', '16%')
					);

	/* employeeNumber: show employeeNumber attribute (matricula) */
	if (authorized_matricula)
		element.append(
					$('<td valign="top">' + areas[area_id].employees[user_id].employeeNumber + '</td>')
						.css('width', '8%')
					);
...
function showGroupedByArea()
{
	...
	if ($('#mailVisibility').attr('checked'))
	   mail = true;
	else
	   mail = false;

	if ($('#categoryVisibility').attr('checked'))
	   category = true;
	else
	   category = false;
		...
		table.append(
				...
				.append(
					$('<td colspan="7"></td>')
						...
						.append(areas[i].sigla + (areas[i].descricao != null ? ' - ' + areas[i].descricao : ''))
					)
		);

		/* creating employee rows */
		for (j=0; j < areas[i].employees.length; j++)
			table.append(createEmployeeRow(i, j, false, photo, mail, category));
	}
...
function showUngrouped()
{
	...
	while (true) {
		...
		/* inserting the row */
		table.append(createEmployeeRow(less, areas[less].index, true, photo, mail, category));
		areas[less].index++;
	}
	$('#areas_content').append(table);
}
...
function bindEvents()
{
	...
	$('#mailVisibility').click(toggleMailVisibility);
	$('#categoryVisibility').click(toggleCategoryVisibility);
