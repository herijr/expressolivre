...
class so_userinterface
{
	...
	{
		if (is_null($parent)){
			$query = "SELECT a.area_id, a.sigla, a.descricao, a.titular_funcionario_id FROM area a";
	...
	/**
	 * Checa se o usuário possui permissão para visualizar informações restritas do organograma (à princípio são: cargo, função e data de admissão).
	...
	public function _checkAccess($organizationID = null)
	{
		...
		{
			// 4 = permissão de visualizar o cargo, 5 = permissão de visualizar a função e 6 = permissão de visualizar a data de admissão
			// Se o usuário possuir todas estas permissões, ou estiver em um grupo que tenha estas permissões, authorized=true.
			$this->authorized = $this->getAuthorizedRestrictedInfo($organizationID, array(4, 5, 6));
		}
	}
	...
	/**
	 * Get authorized value of a specific restricted information
	 * @param int $organizationID O ID da organização do Orgranograma.
	 * @param array $requiredLevel permissoes requeridas
	 * @return boolean
	 * @access public
	 */
	public function getAuthorizedRestrictedInfo($organizationID = null, $requiredLevel = null)
	{
		return $this->acl->checkUserGroupAccessToResource('ORG', $this->userID, (int) $organizationID, (int) $requiredLevel);
	}
	...
	function getAreaEmployees($areaID, $organizationID)
	{
		...
		$authorized_matricula = $this->getAuthorizedRestrictedInfo($organizationID, 3);
		$authorized_cargo = $this->getAuthorizedRestrictedInfo($organizationID, 4);
		$authorized_funcao = $this->getAuthorizedRestrictedInfo($organizationID, 5);
		$authorized_data_admissao = $this->getAuthorizedRestrictedInfo($organizationID, 6);
		...
		for ($i = 0; $i < count($employees); ++$i)
		{
			...
			for ($j = 0; $j < $result->_numOfFields; ++$j)
				unset($employees[$i][$j]);
			if (!$authorized_funcao || !isset($employees[$i]['funcao']))
				$employees[$i]['funcao'] = '';
			else
				$employees[$i]['funcao'] = utf8_encode($employees[$i]['funcao']);
			if (!$authorized_data_admissao || !isset($employees[$i]['data_admissao']))
			...
			if ($entry)
			{
				...
				$employees[$i]['mail'] = is_null($entry['mail']) ? '' : $entry['mail'];
				$employees[$i]['uidnumber'] = is_null($entry['uidnumber']) ? '' : $entry['uidnumber'];
				$employees[$i]['removed'] = is_null($entry['last_update']);

				if ($authorized_matricula)
					$employees[$i]['employeeNumber'] = empty($entry['employeenumber']) ? '' : $entry['employeenumber'];
			}
			...
			if ($authorized_cargo && !empty($employeeInfo['cargo_id']))
			{
