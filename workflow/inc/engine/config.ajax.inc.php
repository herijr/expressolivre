...
// Directory where the Galaxia processes will be stored
if (!defined('GALAXIA_PROCESSES'))
{
		// Note: this directory must be writeable by the webserver !
		define('GALAXIA_PROCESSES', Factory::getInstance('WorkflowObjects')->getBaseDir());
}
...
if (!defined('GALAXIA_LOGFILE')) {
		...
		define('GALAXIA_LOGFILE', GALAXIA_PROCESSES.SEP.'galaxia.log');
}
...
if (!function_exists('galaxia_get_config_values')) 
{
	function galaxia_get_config_values($parameters=array())
	{
			...
			{
				$config_value = isset($config->config_data[$config_var]) ? $config->config_data[$config_var] : NULL;
