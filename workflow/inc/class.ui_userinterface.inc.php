...
class ui_userinterface extends ui_ajaxinterface
{
	...
	function printArea()
	{
		...
		$smarty->assign('authorized_matricula', $so->getAuthorizedRestrictedInfo($organizationID, 3) & 1);
		$smarty->assign('authorized_cargo', $so->getAuthorizedRestrictedInfo($organizationID, 4) & 1);
		$smarty->assign('authorized_funcao', $so->getAuthorizedRestrictedInfo($organizationID, 5) & 1);
		$smarty->assign('authorized_data_admissao', $so->getAuthorizedRestrictedInfo($organizationID, 6) & 1);

		$smarty->assign('authorized', $so->getAuthorized() & 1);
