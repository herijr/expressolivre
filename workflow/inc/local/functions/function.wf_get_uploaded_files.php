...
function wf_get_uploaded_files( $name )
{
	...
	{
		$return[0] = $filesArray;
		$return[0]['content'] = empty($filesArray['tmp_name']) ? '' : file_get_contents($filesArray['tmp_name']);
	}
	else
		for ($i = 0; $i < $numberOfFiles; ++$i)
		{
			...
			$return[$i]['content'] = empty($filesArray['tmp_name'][$i]) ? '' : file_get_contents($filesArray['tmp_name'][$i]);
		}
