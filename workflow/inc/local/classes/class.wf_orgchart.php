...
class wf_orgchart
{
	 ...
	 * - status_descricao: descrição do status (Ativo, Desligado, Aposentado, etc)
	 * - exibir: S ou N, indicando se é exibido no organograma ou não
	...
	function getOrganizationEmployees($organizationID, $searchLdap = false, $onlyActiveUsers = false, $ldapOperationMode = 0)
	{
		$query = "SELECT f.organizacao_id, " .
				 ...
				 "       fs.descricao AS status_descricao, " .
				 "       fs.exibir " .
				 " FROM funcionario f " .
	...
	/**
	 * Busca as áreas de uma organização e inclui os dados da área superior, dados do titular e substituto
	 *
	 * Este método irá buscar na tabela de áreas, todas as áreas que pertencem à organização solicitada, além dos dados da área superior, titulares e substitutos.
	 * @param int $organizationID O ID da organização.
	 * @param int $onlyActiveAreas false= recupera todas as áreas; true= recupera somente as áreas ativas.
	 * @param boolean $searchLdap True, caso seja necessário buscar no LDAP os dados dos usuários. Ou false, caso contrário.
	 * @return array Uma array seqüencial contendo as áreas de uma organização. Cada linha do array conterá:
	 ...
	 * - ativa: indicativo de situação da area, sendo 's' ativa, e 'n' inativa
	 * - auxiliar_funcionario_id: id da secretária da área
	 * - superior_sigla: sigla da área superior
	 * - superior_descricao: nome completo da área superior
	 * - organizacao_nome: sigla da organizacao
	 * - organizacao_descricao: nome completo da organizacao
	 * @access public
	 */
	function getOrganizationAreasFull($organizationID, $onlyActiveAreas = false, $searchLdap = true)
	{
		$query = "SELECT a.organizacao_id, a.area_id, a.area_status_id, " .
				 "       a.superior_area_id, a.centro_custo_id, a.titular_funcionario_id, " .
				 "       a.sigla, a.descricao, a.ativa, a.auxiliar_funcionario_id, " .
				 "		 s.funcionario_id as substituto_funcionario_id, " .
				 "		 asup.sigla as superior_sigla, " .
				 "		 asup.descricao as superior_descricao, " .
				 "		 org.nome as organizacao_nome, " .
				 "		 org.descricao as organizacao_descricao " .
				 " FROM area a " .
				 "  LEFT OUTER JOIN substituicao s " .
				 "  ON ((a.area_id = s.area_id) AND (CURRENT_DATE BETWEEN s.data_inicio AND s.data_fim)) " .
				 "  LEFT OUTER JOIN area asup " .
				 "  ON asup.area_id = a.superior_area_id " .
				 "  LEFT OUTER JOIN organizacao org " .
				 "  ON org.organizacao_id = a.organizacao_id " .
				 " WHERE " .
				 "	(a.organizacao_id = ?) ";
		// Se desejar somente retornar as áreas que estão ativas
		if($onlyActiveAreas)
		...
		if($searchLdap){
			$output = $this->searchEmployeeDataInLdap($output, array('titular_funcionario_id'=>'titular_', 'substituto_funcionario_id'=>'substituto_'));
		}
...
//			$output[$i]['descricao'] = utf8_encode($output[$i]['descricao']);
	 ...
	 * @return array de arrays associativas contendo a lista de organizações. Cada linha do array conterá:
	 * - organizacao_id
	 ...
	 */
	function searchEmployeeDataInLdap($output, $itens = array('funcionario_id'=>''))
	{
		if(is_array($output)){
			foreach($output AS $k => $value){
				foreach($itens AS $coluna => $prefixo){
					if(is_numeric($value[$coluna])){
						$user_data = $this->ldap->getEntryByID($value[$coluna]);
						$output[$k][$prefixo.'nome']     = $user_data['cn'];
						$output[$k][$prefixo.'email']    = $user_data['mail'];
						$output[$k][$prefixo.'telefone'] = $user_data['telephonenumber'];
						$output[$k][$prefixo.'uid']      = $user_data['uid'];
					}
				}
			}
		}
