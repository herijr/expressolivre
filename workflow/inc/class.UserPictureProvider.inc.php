...
class UserPictureProvider
{
	...
	*/
	...
	/**
	 * @var string $basedir Base Directory Workflow
	 * @access private
	 */
	private $basedir;
	...
	public function UserPictureProvider()
	{
		...
		$this->basedir = Factory::getInstance('WorkflowObjects')->getBaseDir();
	}
	...
	public function serve()
	{
		...
		$baseDirectory = str_replace('//', '/', $this->basedir . '/workflow/ldapPictures');
		$filename =  $baseDirectory . '/' . $this->userID;

		$createPictureFile = true;
		if (file_exists($filename))
		{
			if ((filemtime($filename) + ($this->cacheDays * 24 * 60 * 60)) > time())
