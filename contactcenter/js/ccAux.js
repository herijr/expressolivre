			...
			else switch(typeof(data))
			{
				case 'object':
					...
					for (i in data)
					{
						...
						{
							case 'number':
								str_data += 'i:' + i + ';' + ccAux.serialize(data[i]);
								break;

							case 'string':
								str_data += 's:' + i.length + ':"' + i + '";' + ccAux.serialize(data[i]);
								break;
			...
			switch (str.charAt(0))
			{
				case 'a':
					...
					for (var i = 0; i < n; i++)
					{
						...
						var index = ccAux.unserialize(indexStr);
						...
						switch (arrayContent.charAt(pos))
						{
							case 'a':
								...
								pos = pos_;
								data[index] = ccAux.unserialize(part);
								break;
