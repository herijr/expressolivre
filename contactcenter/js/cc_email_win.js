  ...
  *  option) any later version.                                               *
  \***************************************************************************/
	...
	ccEmailWinClass.prototype.open = function(search)
	{
		if (!search) {
			...
			this.search.DOMinput.value = search;
			this.catalogues.select("0.2", search);
			this.Connector.setVisible(true);
			this.window.open();
		}
	}
...
//		this.clearEntries();
	...
	ccEmailWinClass.prototype._search_set = function()
	{
		...
		this.search.onSearchFinish = function (result)
		{
			...
			var str_data = 'data='+ccAux.serialize(sdata);
	...
	ccEmailWinClass.prototype._updateEntries = function (data)
	{
		...
		for (var i in data)
		{
			...
			for (var j in data[i])
			{
				...
				else if (j == 'short_name')
				{
							emails[emails.length] = ' ('+data[i][j]+')';
				}
			}
