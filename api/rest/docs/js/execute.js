...
function getResourcesExpresso(s_callback) {
	...
	$.ajax({
		...
		data	:
		{
	   			id			: "1",
	   			params 		: {} ,
	   			serverUrl	: server + "Documentation",
	   			methodType	: "POST"
			},
			beforeSend: function()
			{
...
function loadPage(pageName,param) {
	...
	var s_callback = function (response) {
        ...
        var possible_errors = response.possible_errors;

		divResources = $("#contentDetail");

		divResources.html( new EJS({ url: './templates/' + pageName + '.ejs'},false).render({ params : param, resources : resources_expresso, possible_errors : possible_errors }));
...
function selectResource(resourceName) {
	...
	var s_callback = function (response) {
		...
		// for( var i in resources_expresso )
		// {
			// $("#sub_"+resources_expresso[i].id).accordion(
			// {
			// 	collapsible: true,
			// 	heightStyle: "content"
			// });
	...
	{  
        ...
        var s_callback = function (response) {
            ...
            var resources_expresso = response.resources;
    			...
    			{
    				var server = $('#serverAPI').val() + "/" + resources_expresso[rest].rest;
    				...
    				$.ajax(
    				{
    					...
    					data	:
    					{
    	  				...
    	  				beforeSend: function()
    	  				{
    	  					...
    	  					//divSend.accordion({ collapsible: true, heightStyle: "content", collapsible : true });
    	  					...
    	  					//divSend.find("div").find("pre").css("border","0px");
    	  				},
                     	success: function(response)
                     	{
    	          	 		var obj = jQuery.parseJSON(response);
    	          	 		...
    	          	 		else
    	          	 		{
    	                 		...
    	                 		//divReturn.accordion({ collapsible: true, heightStyle: "content", collapsible: true });

    	                 		//divReturn.toggle("blind", {}, 1000 );
