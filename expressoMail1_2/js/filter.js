	...
	cfilter.prototype.forms_ = function()
	{
			 var form = "";
				 form = "<div id='div_rule' style='display:none'><table id='table_rule' border='0' cellpading='0' cellspacing='0' width='100%'>"+
					 ...
					 "<option value='0'>"+get_lang("If any criterion is met")+"</option><option value='1'>"+get_lang("If all criteria is met")+"</option>"+
					 "</select></td><td>"+get_lang('The field \"%1\" of the message it contains',get_lang('From'))+".: <input type='text' id='field1' name='field1' size='35' maxlength='200'></td>"+
						...
						"<tr><td colspan='3'><br>"+get_lang('With the following message')+".:</td>"+
	 					"</tr><tr><td colspan='3'><textarea id='field8' onkeypress='return charCounter(this);' rows='8' cols='50'></textarea></td></tr></table></div>" +
						...
						"</td></tr><tr><td><input type='text' id='field9' size='50' maxlength='200'>" +
						"</td></tr><br/></table></div>" +
...
cfilter.prototype.new_rule = function(email){
	...
	var createFilter = function (param){ 
		...
		if(confirm(get_lang("Do you want to block this e-mail?"))){ 
			new_r = "&&ENABLED&&" + param + "&&&&&&discard&&&&0&&&&&&0"; 
			//setTimeout("filter.e_newrule('" + new_r + "')",2000);          
	...
	cfilter.prototype.saved_rules = function(){
		...
		if(form[1] == "rule")
		{
			...
			if ( 
				( /[^\x00-\x80]/.test( Element("field1").value ) || /[^\x00-\x80]/.test( Element("field2").value ) || /[^\x00-\x80]/.test( Element("field3").value ) )
				&& ( ! confirm( get_lang ( 'Your rule contains any accented character. The filter may not work. Do you wish to continue?' ) ) )
			) return false;

			if(LTrim(Element("field1").value) == "" && LTrim(Element("field2").value) == "" && LTrim(Element("field3").value) == "" && Element("field4").value == "")
			{
				alert(get_lang("Define some criterion to the fields Sender, To and Subject with more than 3 characters!"));
				return false;
			}

			if((LTrim(Element("field1").value).length <= 3) && (LTrim(Element("field2").value).length <= 3) && (LTrim(Element("field3").value).length <= 3 && Element("field4").value == "")){
				alert(get_lang("Define some criterion to the fields Sender, To and Subject with more than 3 characters!"));
				return false;
			}
