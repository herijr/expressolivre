...
class imap_functions
{
	...
	function get_info_head_msg($msg_number)
	{
		...
		//{
			/*Como eu preciso do atributo Importance para saber se o email �
			 * importante ou n�o, uso abaixo a fun��o imap_fetchheader e busco
			 * o atributo importance nela. Isso faz com que eu acesse o cabe�alho
			 * duas vezes e de duas formas diferentes, mas em contrapartida, eu
			 * n�o preciso reimplementar o m�todo utilizando o fetchheader.
			 * Como as mensagens s�o renderizadas em um n�mero pequeno por vez,
			 * n�o parece ter perda consider�vel de performance.
			 */
			...
			$flag = preg_match('/importance *: *(.*)\r/i', $tempHeader, $importance);
                //}
                // Reimplementado c�digo para identifica��o dos e-mails assinados e cifrados
                // no m�todo getMessageType(). M�rio C�sar Kolling <mario.kolling@serpro.gov.br>
...
//		else
		...
		$head_array['to'] = array();
		if( isset($to[1]) && isset( $to[1]->host ) && $to[1]->host == ".SYNTAX-ERROR.") { //E-mails que n�o possuem o campo "para", v�m com o recipiente preenchido, por�m com um recipiente a mais alegando erro de sintaxe.
	...
	/**
	* Fun��o que importa arquivos .eml exportados pelo expresso para a caixa do usu�rio. Testado apenas
	...
	*/
	function remove_attachments($params) {
		...
		$msg->fetch_structure($params["msg_num"]);/* O fetchbody tava trazendo o email com problemas na acentua��o.
							     Ent�o uso essa classe para verificar a codifica��o e o charset,
		...
		if (($all_body_charset == "utf-8") && ($all_body_encoding == "base64")){
			$status = imap_append($this->mbox,
					"{".$this->imap_server.":".$this->imap_port.$this->imap_options."}".$params["folder"],
						...
						, "\\Seen"); //Append do novo email, s� com header e conte�do sem anexos.			
		}else{	
			$status = imap_append($this->mbox,
					"{".$this->imap_server.":".$this->imap_port.$this->imap_options."}".$params["folder"],
						$header.
...
//		else
	...
	function replace_special_characters($body)
	{
		...
		*/
		...
		//Remo��o de tags <span></span> para corre��o de erro no firefox 
		$body = mb_eregi_replace("<span><span>","",$body); 
		$body = mb_eregi_replace("</span></span>","",$body); 
		//Corre��o para compatibiliza��o com Outlook, ao visualizar a mensagem 
	...
	function replace_links( $body )
	{
		...
		// PHP 5.2.x - Remover assim que poss�vel 
	...
	function refresh($params)
	{
		...
		{
			/*A fun��o imap_headerinfo n�o traz o cabe�alho completo, e sim alguns
			* atributos do cabe�alho. Como eu preciso do atributo Importance
			* para saber se o email � importante ou n�o, uso abaixo a fun��o
			* imap_fetchheader e busco o atributo importance nela para passar
			* para as fun��es ajax. Isso faz com que eu acesse o cabe�alho
			* duas vezes e de duas formas diferentes, mas em contrapartida, eu
			* n�o preciso reimplementar o m�todo utilizando o fetchheader.
			*/
	...
	/**
	 * M�todo que faz a verifica��o do Content-Type do e-mail e verifica se � um e-mail normal,
	 * assinado ou cifrado.
	 * @author M�rio C�sar Kolling <mario.kolling@serpro.gov.br>
	 * @param $headers Uma String contendo os Headers do e-mail retornados pela fun��o imap_imap_fetchheader
     ...
     * Se usar $params['noSharedFolders'] = true, ira retornar todas as pastas do usu�rio logado,
     * excluindo as compartilhadas para ele.
     * Se usar $params['folderType'] = "default" ir� retornar somente as pastas defaults
     * Se usar $params['folderType'] = "personal" ir� retornar somente as pastas pessoais
     * Se usar $params['folderType'] = null ir� retornar todas as pastas
	...
	function send_mail($params)
	{
        ...
        {
            $body = $params['body'];
	    //Compatibiliza��o com Outlook, ao encaminhar a mensagem
	    $body = mb_ereg_replace('<!--\[','<!-- [',$body);
        }
...
///expressoMail1_2/imap.py para inserir uma msg com o hor�rio correto pois isso n�o � porss�vel com a fun��o imap_append do php.
...
//Base64 os "+" s�o substituidos por " " no envio e essa fun��o arruma esse efeito.
...
//Por Bruno Costa(bruno.vieira-costa@serpro.gov.br - Recebe os fontes dos emails a serem desarquivados, separa e envia cada um para fun��o insert_mail.
