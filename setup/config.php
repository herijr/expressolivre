  ...
  *  option) any later version.                                              *
  \**************************************************************************/
	...
	*/
	...
	{
		if( !empty( $newsettings['temp_dir'] ) )
		{
			// Retira a ultima barra;
			if( substr_count( $newsettings['temp_dir'] , "/") > 1 )
			{
				$pos = strrpos( $newsettings['temp_dir'] , "/" );
				
				$newsettings['temp_dir'] = substr( $newsettings['temp_dir'] , 0 , $pos);
			}
		}
	...
	@$GLOBALS['phpgw_setup']->db->query("SELECT * FROM $configtbl where config_app='phpgwapi'");
