...
class sector_search_ldap {
	...
	function get_organizations($context, $selected='', $referral=false, $show_invisible_ou=false, $master=false, $filter_config=false) {
		if ($master) {
			...
			$dn = $this->_server_conf['ldap_root_dn'];
			$passwd = $this->_server_conf['ldap_root_pw'];
			$host = $this->_server_conf['ldap_host'];
		}
	...
	function get_sectors($selected='', $referral=false, $show_invisible_ou=false, $filter_config=false) {
		$dn = $this->_server_conf['ldap_root_dn'];
		$passwd = $this->_server_conf['ldap_root_pw'];
		$context = $this->_server_conf['ldap_context'];
		$ldap_conn = ldap_connect($this->_server_conf['ldap_host']);
		...
		for ($i = 0; $i < $info["count"]; $i++) {
			...
			if ( !($filter_config && $this->denied( $dn )) ) {
				$select = ($dn == $selected)? ' selected' : '';
				$display = strtoupper($info[$i]['ou'][0]);
				$options .= "<option value='$dn'$select>$display</option>";
			}
		}
		
		return (empty($options))? "<option value='$context' selected> $context </option>" : $options;
	}
