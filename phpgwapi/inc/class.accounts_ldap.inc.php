	...
	class accounts_
	{
		...
		function read_repository()
		{
			...
			if(($acct_type == 'g') && $this->group_context)
			{
				...
				$sri = @ldap_search($this->ds, $this->group_context, ("(&(gidnumber=" . (int)$this->account_id.")(phpgwaccounttype=g))"), $justthese);
			}
			else
			{
				/* jakjr: using justthese with ldap_search */
				$justthese = array("uidnumber", "gidnumber", "uid", "cn", "givenname", "sn", "mail", "phpgwaccountstatus", "phpgwaccountexpires", "phpgwaccounttype", "phpgwlastpasswdchange","phpgwagreeterm","telephonenumber","mobile","homephone","datanascimento");
				$sri = @ldap_search($this->ds, $this->user_context, ("(&(uidnumber=" . (int)$this->account_id.")(phpgwaccounttype=u))"), $justthese);
			}
			if(!$sri)
				return null;
		...
		function save_repository()
		{
			...
			if(($acct_type == 'g') && $this->group_context)
			{
				$sri = ldap_search($this->ds, $this->group_context, ("(&(gidnumber=" . (int)$this->account_id.")(phpgwaccounttype=g))"));
			}
			else
			{
				$sri = ldap_search($this->ds, $this->user_context, ("(&(uidnumber=" . (int)$this->account_id.")(phpgwaccounttype=u))"));
			}
			$allValues = ldap_get_entries($this->ds, $sri);
		...
		function delete($accountid = '')
		{
			...
			$filter = "(&(gidnumber=" . (int)$account_id.")(phpgwaccounttype=g))";
		...
		function id2name($account_id,$which='account_lid')
		{
			if ($which == 'account_lid' || $which == 'account_type')	// groups only support account_lid and account_type
			{
				$allValues = array();
				/* jakjr: using justthese with ldap_search */
				$sri = @ldap_search($this->ds, $this->group_context, '(&(gidnumber=' . (int)$account_id . ')(phpgwaccounttype=g))', array("cn"));
				if(!$sri)
					return False;
				$allValues = ldap_get_entries($this->ds, $sri);
			...
			$allValues = array();
			/* jakjr: using justthese with ldap_search */
			$justthese = array("uid","mail","surname","cn","phpgwaccounttype");
			$sri = @ldap_search($this->ds, $this->user_context, '(&(uidnumber=' . (int)$account_id . ')(phpgwaccounttype=u))', $justthese);
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
		...
		function get_type($account_id)
		{
			$allValues = array();
			/* jakjr: using justthese with ldap_search */
			$sri = @ldap_search($this->ds, $this->user_context, '(&(uidnumber=' . (int)$account_id . ')(phpgwaccounttype=u))', array("phpgwaccounttype"));
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
			...
			$allValues = array();
			/* jakjr: using justthese with ldap_search */
			$sri = @ldap_search($this->ds, $this->group_context, '(&(gidnumber=' . (int)$account_id . ')(phpgwaccounttype=g))', array("phpgwaccounttype"));
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
		...
		function get_account_name($account_id,&$lid,&$fname,&$lname)
		{
			...
			if(($acct_type == 'g') && $this->group_context)
			{
				$sri = @ldap_search($this->ds, $this->group_context, '(&(gidnumber=' . (int)$account_id . ')(phpgwAccountType=g))', $justthese);
			}
			else
			{
				$sri = @ldap_search($this->ds, $this->user_context, '(&(uidnumber=' . (int)$account_id . ')(phpgwAccountType=u))', $justthese);
		...
		//by JakJr, add organization support
                function get_context($dn)
		{
			return substr($dn, (strpos($dn, ',')+1));
		}

		//by JakJr, add organization support
		function get_organization($dn)
		{
			$ldap_context = $GLOBALS['phpgw_info']['server']['ldap_context'];
			$end = (strpos($dn, $ldap_context)-1);
			$tmp =  substr($dn, 0, $end);
			$begin = (strrpos($tmp, '=')+1);
			return substr($dn, $begin, ($end - $begin));
		}

		//by JakJr, add organization support
		function get_sector($dn)
		{
			$context = $this->get_context($dn);
			$begin = (strpos($context, '=')+1);
			$end = (strpos($context, ',') - $begin);
			return substr($context, $begin, $end);
		}
		...
		function mail2fullname($mail)
                {
			$allValues = array();
			$sri = @ldap_search($this->ds, $this->user_context, '(&(mail=' . $mail . ')(phpgwaccounttype=u))');
			if(!$sri)
				return False;
			$allValues = ldap_get_entries($this->ds, $sri);
			if (@$allValues[0]['cn'][0])
