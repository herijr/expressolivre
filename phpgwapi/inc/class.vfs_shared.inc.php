  ...
  * Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA            *
  \**************************************************************************/
	...
	class vfs_shared
	{
		...
		* @owner_id string	uid owner
		*/
		
		function get_size_all($owner_id) { return 0; }
		...
		function getabsolutepath ($data)
		{
			...
			/* Let's not return // */
			while (preg_match('#'.$sep . $sep.'#', $basedir))
			{
				$basedir = preg_replace('#'.$sep . $sep.'#', $sep, $basedir);
			}

			$basedir = preg_replace('#'.$sep . '$#', '', $basedir);
		...
		function path_parts ($data)
		{
			...
			if (!$data['nolinks'])
			{
				...
				{
					if (preg_match("#^$link_info[directory]/$link_info[name](/|$)#", $rarray['fake_full_path']))
					{
						$rarray['real_full_path'] = preg_replace("#^$this->basedir#", '', $rarray['real_full_path']);
						$rarray['real_full_path'] = preg_replace("#^$link_info[directory]" . SEP . "$link_info[name]#", $link_info['link_directory'] . SEP . $link_info['link_name'], $rarray['real_full_path']);
