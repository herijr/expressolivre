...
//			$this->tbl_config  = $this->get_config_table_name();
		...
		*/
		function get_major($versionstring)
		{
			...
			$version = str_replace('-','',str_replace('pre','.',$versionstring));
			$varray  = explode('.',$version);
			$major   = implode('.',array($varray[0],$varray[1]));
		...
		*/
		...
		function setup_account_object()
		{
			if (!is_object($GLOBALS['phpgw']->accounts))
			{
				...
				$this->db->query("SELECT config_name,config_value,config_app FROM phpgw_config "
		...
		*/
		function add_acl($apps,$location,$account,$rights=1)
		{
			...
			$this->db->query("END TRANSACTION; COMMIT;");
			if ($this->db->Error){
				echo "Error in acl update";
				exit;
			}
		}
	}
