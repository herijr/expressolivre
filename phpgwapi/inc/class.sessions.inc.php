  ...
  \**************************************************************************/
	...
	*/
	...
	class sessions_
	{
		...
		*/
		function log_access( $sessionid, $login = '', $user_ip = '', $account_id = '', $is_invalid = false )
		{
			$now = time();
			
			if ( !$is_invalid ) $this->logout_access( null, $sessionid, $now );
			
			if ( $login === '' ) return false;
			
			if ( strlen( $login ) > 30 ) $login = substr( $login, 0, 30 );
			
			$this->db->query( '
				INSERT INTO phpgw_access_log( sessionid, loginid, ip, li, lo, account_id, browser )
				VALUES (
					\''.$this->db->db_addslashes( $sessionid ).'\',
					\''.$this->db->db_addslashes( $login ).'\',
					\''.$this->db->db_addslashes( $user_ip ).'\',
					'.$now.', '.( $is_invalid? -1 : 0 ).', '.(int)$account_id.',
					\''.$this->db->db_addslashes( substr( $_SERVER['HTTP_USER_AGENT'], 0, 199 ) ).'\'
				)', __LINE__, __FILE__
			);
		...
		public function logout_access( $id = null, $sid = null, $time = null )
		{
			$this->db->query( '
				UPDATE phpgw_access_log
				SET lo = \''.( is_null( $time )? time() : (int)$time ).'\'
				WHERE lo = 0 '.
					( ( is_null( $id  ) || $id  === false )? '' : 'AND loginid   = \''.pg_escape_string( $id  ).'\'' ).
					( ( is_null( $sid ) || $sid === false )? '' : 'AND sessionid = \''.pg_escape_string( $sid ).'\'' )
				, __LINE__, __FILE__
			);
			return true;
		}
