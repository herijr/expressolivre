	...
	class nextmatchs
	{
		...
		function nextmatchs($website=False)
		{
			...
			foreach(array('menuaction','filter','qfield','order','sort') as $name)
			{
				...
				if ( !( is_string( $this->$var ) && preg_match( '/^[a-z0-9_. -]*$/i', $this->$var ) ) )
					$this->$var = '';
			}
		...
		*/
		function set_link($align,$img,$link,$alt,$extravars)
		{
			$var = Array(
				'align'     => $align,
				'action'    => ($this->_menuaction?$this->page($extravars):$GLOBALS['phpgw']->link($link)),
				'form_name' => $img,
		...
		*/
		function left($scriptname,$start,$total,$extradata = '')
		{
			...
			{
				$ret_str .= $this->set_icon('left','first-grey.png','First page');
				$ret_str .= $this->set_icon('left','left-grey.png','Previous page');
			}
			return $ret_str;
		} /* left() */
		...
		*/
		function right($scriptname,$start,$total,$extradata = '')
		{
				...
				($total > $start + $this->maxmatches))
			{
				...
				$num_regs_last_page = $total % $this->maxmatches;
				$extravars['start'] = ($total - $num_regs_last_page);
				$ret_str .= $this->set_link('right','last.png',$scriptname,lang('Last page'),$extravars);
			}
			else
			{
				$ret_str .= $this->set_icon('right','right-grey.png','Next page');
				$ret_str .= $this->set_icon('right','last-grey.png','Last page');
			}
			return $ret_str;
		} /* right() */
