...
//				_debug_array($column);
		...
		*/
		...
		function persist_shared_groups($location,$account_id)
		{

			$sql = "delete from phpgw_cc_contact_grps where oid in (
				select A.oid from phpgw_cc_contact_grps A inner join phpgw_cc_groups B on A.id_group=B.id_group 
				inner join phpgw_cc_contact_conns C on A.id_connection=C.id_connection 
				inner join phpgw_cc_contact D on C.id_contact=D.id_contact
				where B.owner=$location and D.id_owner=$account_id );";
