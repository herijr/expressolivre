...
Calendar.prototype.create = function (_par) {
	...
	if (this.isPopup) {
		...
		div.style.display = "none";
                //TODO: tornar zIndex uma das opções de inicialização
                div.style.zIndex = "100000"; // hardcoded,
	}
...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	for (var i = 0; i < 6; ++i, row = row.nextSibling) {
		...
		for (var j = 0; j < 7; ++j, cell = cell.nextSibling, date.setDate(date.getDate() + 1)) {
                        ...
                        // Para evitar repetição de dias quando a instrução date.setDate(date.getDate() + 1)
                        // não incrementa a variável date corretamente.
                        if (!first && date.getDate() == iday)
                        {
...
Calendar.prototype.show = function () {
	...
	this.hideShowCovered();
	var today = new Date();
	// See tickets #252 and #284 in the tracking of expressolivre.org.
	// The line below was discussed to solve these problems.
	//this._init(1,today);
...
Calendar.prototype.hideShowCovered = function () {
	...
	Calendar.continuation_for_the_fucking_khtml_browser = function() {
		...
		for (var k = tags.length; Calendar.is_ie6 && k > 0; ) {
