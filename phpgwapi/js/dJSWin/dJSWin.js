...
/* Global array for created windows */
var arrayJSWin = new Array();
...
if (!window.__DEFINE_DJSWIN__)
{
	...
	function dJSWin(params)
	{
		...
		if (params['button_y_img'])
		{
			var _this = this;
			
			this.button_ext = document.createElement('IMG');
			this.button_ext.id = 'button_ext';
			this.button_ext.src = params['button_y_img'];
			this.button_ext.style.position = 'absolute';
			this.button_ext.style.cursor = 'pointer';
			this.button_ext.style.top = '1px';
			this.button_ext.style.left = parseInt(params['width']) - 28	 + 'px';
			this.button_ext.style.zIndex = '1';
			this.button_ext.onclick = function(){_this.hide(); _this.state = 1;};
		}
...
//		this.clientArea.style.overflow = 'auto';
		...
		this.buttons.xDIV.onclick = function() {_this.close(); _this.state = 2;};
...
*/
		...
		if(this.button_ext){this.title.appendChild(this.button_ext);};		
		...
		/* Add new window to the array */
		arrayJSWin[params.content_id] = this;
	}
	...
	dJSWin.prototype.close = function()
	{
		...
		/* Turn the attribute visible false */
		this.visible = false;
	}
...
*/
		...
		/* Turn the attribute visible true */
		this.visible = true;
	}
