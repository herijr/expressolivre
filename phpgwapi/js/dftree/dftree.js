 ...
 */
/*
 * Chanded by Joao Alfredo Knopik Junior
...
dNode.prototype._select = function()
{
	...
	if(document.getElementById("div_tree") != null){
		if(ttree.FOLDER != ""){
			ttree.FOLDER = "";
		}
		ttree.FOLDER = this.id;
	}
	
	if(document.getElementById("div_folders_search") != null){
		if(EsearchE.name_box_search != ""){
			EsearchE.name_box_search = "";
		}
		EsearchE.name_box_search = this.id;
	}
...
//changes node background color.
dNode.prototype._onMouseOver = function()
{
	var captionSpan;
	if ((this.id != 'root') && (this.id != 'user') && (this.id !='local_root')){
		captionSpan = getObjectById("l"+this.id+this._myTree.name);
		captionSpan.style.backgroundColor = 'white';
		captionSpan.style.border = '1px solid black';
		captionSpan.style.paddingTop = '0px';
		captionSpan.style.paddingBottom = '0px';
	}
}
//changes node background color.
dNode.prototype._onMouseOut = function()
{
	var captionSpan;
	if ((this.id != 'root') && (this.id != 'user') && (this.id !='local_root')){
		captionSpan = getObjectById("l"+this.id+this._myTree.name);
		captionSpan.style.backgroundColor = '';
		captionSpan.style.border = '0px';
		captionSpan.style.paddingTop = '1px';
		captionSpan.style.paddingBottom = '1px';
	}
}
...
/*	if (this.myTree.followCookies)
	...
	//divN.className = 'son';
	divN.className = (!this._parent) ? 'root' : 'son';
	...
	spanL.onmouseover = function () { _this._onMouseOver(); };
	spanL.onmouseout = function () { _this._onMouseOut(); };
//	spanL.style.border = '1px solid #f7f7f7';
...
dFTree.prototype.add = function(node,pid) {
	...
	if (typeof (auxPos = this._searchNode(node.id)) != "number")
	{
		...
		{
			this._folderPr[this._folderPr.length] = node.id ;
		}
...
// By jakjr, retorna um array com os ids de todas as pastas (nodes)
dFTree.prototype.getNodesList = function(imapDelimiter) {
	var a=0;
	var nodes=[];
	for (a;a<this._aNodes.length;a++)
	{
		var node = this.getNodeById(this._aNodes[a].id);
		nodes[a]=[];
		nodes[a].id = node.id;
		nodes[a].parent = node._parent ? node._parent.id : 'root';		
		var tmp = node.id.split(imapDelimiter);
		var tmp_caption = node.caption.split("<");

		if (node.id == 'INBOX')
			nodes[a].caption = get_lang('Inbox');
		else if (node.id == 'user')
			nodes[a].caption = get_lang("Shared Folders");
		else
			nodes[a].caption = tmp_caption[0];
		//nodes[a].caption = tmp[tmp.length-1] == 'INBOX' ? get_lang('Inbox') : tmp[tmp.length-1];
		nodes[a].plusSign = node.plusSign;
	}
	return nodes;
}
