...
include_once(PHPGW_API_INC.'/class.aclmanagers.inc.php');
	...
	class uireports_institutionalAccounts
	{
		...
		(
			'report_institutionalAccounts_print_pdf'		=> True,
			'report_institutionalAccounts_group'			=> True,
			'report_institutionalAccounts_group_setor_print'	=> True,
			'get_institutionalAccount_info'				=> True,
			'css'						=> True
		);
		...
		function uireports_institutionalAccounts()
		{
		...
		function report_institutionalAccounts_print_pdf()
		{
			...
			//Table header
			$SubTitulo = lang('Report of institutional accounts of the Organization');
			...
			$account_info = $this->functions->get_list_institutionalAccounts_sector($setordn,$contexts,0);
			...
			{ 
				...
				$pdf->Cell(80,5,lang('report email'),1,0,'L',1);
				$pdf->Cell(20,5,lang('status'),1,0,'L',1);
	                        $pdf->Cell(80,5,lang('Participants'),1,1,'L',1);
				...
				while (list($null,$account) = each($account_info))
				{
						...
						$row_mailforwardingaddress = $account['account_mailforwardingaddress'];
						
						$pdf->Cell(80,5,$row_cn,0,0,'L',0);
						$pdf->Cell(80,5,$row_mail,0,0,'L',0);
					...
					// Lista os participantes da conta institucional
					$max = count($row_mailforwardingaddress);
					for ($i=0; $i <= $max; $i++)
					{
						if ($max == $i)
					        {
							$pdf->Cell(80,5,$row_mailforwardingaddress[$i],0,1,'L',0);
						} 
						else
						{
							$pdf->Cell(80,5,$row_mailforwardingaddress[$i],0,2,'L',0);
						}
					}
		...
		function report_institutionalAccounts_group()
		{
			$account_lid = $GLOBALS['phpgw']->accounts->data['account_lid'];
			...
			$GLOBALS['phpgw_info']['flags']['app_header'] = $GLOBALS['phpgw_info']['apps']['reports']['title'].' - '.lang('Report institutional accounts by Organization');
			$GLOBALS['phpgw']->common->phpgw_header();

			$p = CreateObject('phpgwapi.Template',PHPGW_APP_TPL);
			$p->set_file(array('groups'   => 'report_institutionalAccounts_group.tpl'));
			...
			if ($GLOBALS['organizacaodn'] != '')
			{
				...
				$account_user = $this->functions->get_count_institutionalAccount_sector($contextsdn,$contexts,0);
		...
		function report_institutionalAccounts_group_setor_print()
		{
			$grouplist = trim($_POST[setor]);
			$grouplist = trim(preg_replace("/-/","",$grouplist));
			...
			$p = CreateObject('phpgwapi.Template',PHPGW_APP_TPL);
			$p->set_file(Array('accounts' => 'report_institutionalAccounts_group_print.tpl'));
			...
			{ 
				//url do paginador 
				$url = $GLOBALS['phpgw_info']['server']['webserver_url'].'/index.php?menuaction=reports.uireports_institutionalAccounts.report_institutionalAccounts_group_setor_print';
				...
				$paginas = $this->functions->Paginate_institutionalAccount('accounts',$setordn,$contexts,'cn','asc',$npag,$numpage);
				...
				while (list($null,$accountp) = each($paginas))
				{
					...
					$tmpp[$accountp['uid'][0]]['account_mailForwardingAddress']      = $accountp['mailForwardingAddress'];
				...
				while (list($null,$accountr) = each($returnp))
				{
					$this->nextmatchs->template_alternate_row_color($p);
                                        ...
                                        $max = count($accountr['account_mailForwardingAddress']);
					$row_participants = '';
                                        for ($i=0; $i <= $max; $i++)
                                       {
                                                if (0 == $i)
                                                {
                                                        $row_participants = $accountr['account_mailForwardingAddress'][$i];
                                                }
						elseif ($max == $i)
						{
							$row_participants = $row_participants . $accountr['account_mailForwardingAddress'][$i];
						}
                                                else
                                                {
		...
		function get_institutionalAccount_info($userdn,$usercontexts,$usersizelimit)
		{
			$user_info = $this->functions->Paginate_institutionalAccount('accounts',$setordn,$contexts,'cn','asc',$npag,$numpage);
