...
include_once(PHPGW_API_INC.'/class.aclmanagers.inc.php');
	...
	class uireports_usersgroups
	{
		var $public_functions = array
		(
			'report_usersgroups_group_print_pdf'	=> True,
			'report_usersgroups_group'				=> True,
			'report_usersgroups_group_print'		=> True,
			'get_usersgroups_info'					=> True,
			'css'									=> True
		);
		...
		function uireports_usersgroups()
		{
		...
		function report_usersgroups_group_print_pdf()
		{
			...
			$pdf->Open();
			$pdf->AddPage();
			$pdf->SetTitle(lang('Report of institutional accounts of the Organization'));
			...
			$account_info = $this->functions->get_list_usersgroups_sector($setordn,$contexts,0);
			...
			{ 
				...
				$pdf->Cell(40,5,lang('report id'),1,0,'L',1);
				$pdf->Cell(75,5,lang('name'),1,0,'L',1);
				$pdf->Cell(75,5,lang('report description'),1,1,'L',1);
				
				while (list($null,$account) = each($account_info))
				{
					$pdf->Cell(40,5,$account['id'],0,0,'L',0);
					$pdf->Cell(75,5,$account['name'],0,0,'L',0);
					$pdf->Cell(75,5,$account['description'],0,1,'L',0);
				}
			}
		...
		function report_usersgroups_group()
		{
			$account_lid = $GLOBALS['phpgw']->accounts->data['account_lid'];
			...
			$p->set_file(array('groups'   => 'report_usersgroups_group.tpl'));
			...
			if ($GLOBALS['organizacaodn'] != '')
			{
				...
				$account_usersgroups = $this->functions->get_list_usersgroups_sector($contextsdn,$contexts,0);
				$total_usersgroups = "(".count($account_usersgroups).")";

				$p->set_var('organizacao', $varorganizacao_nome);
				$p->set_var('all_usersgroups', lang('all'));
				$p->set_var('total_usersgroups', $total_usersgroups);
		...
		function report_usersgroups_group_print()
		{
			$grouplist = trim($_POST[setor]);
			$grouplist = trim(preg_replace("/-/","",$grouplist));
			...
			if (!count($account_info))
			{
				$p->set_var('message',lang('No usersgroups found'));
				$p->parse('rows','row_empty',True);
			}
			...
			{ 
				...
				$p->set_var('cont_usersgroups',$numreg);
				$p->set_var('cont_page',$tp);
				$p->set_var('page_now',$npag);
				
				// ********** busca no LDAP as informação paginada e imprime ****************
				$paginas = $this->functions->paginate_usersgroups($setordn,$contexts,'cn','asc',$npag,$numpage);
				
				$tmpp = array();
				
				while (list($null,$accountp) = each($paginas))
				{
					$tmpp[$accountp['cn'][0]]['id']	 			= $accountp['gidNumber'][0]; 
					$tmpp[$accountp['cn'][0]]['name']	 		= $accountp['cn'][0];
					$tmpp[$accountp['cn'][0]]['description']	= $accountp['description'][0];
					$sortp[] = $accountp['cn'][0];
					if (count($sortp))
					{
						natcasesort($sortp);
				...
				while (list($null,$accountr) = each($returnp))
				{
					...
					$varr = array(
						'row_id'			=> $accountr['id'],
						'row_name'			=> $accountr['name'],
						'row_description'	=> $accountr['description']
					);
