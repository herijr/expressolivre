...
include_once(PHPGW_API_INC.'/class.aclmanagers.inc.php');
	...
	class uireports_users
	{
		var $public_functions = array
		(
			'report_users_print_pdf'			=> True,
			'report_users_group'				=> True,
			'report_users_group_setor_print'	=> True,
			'get_user_info'						=> True,
			'css'								=> True
		);
		...
		function report_users_print_pdf()
		{
			...
			$account_info = $this->functions->get_list_user_sector($setordn,$contexts,0);
			...
			{ 
				...
				$pdf->Cell(35,5,lang('report phone'),1,0,'L',1);
				$pdf->Cell(20,5,lang('status'),1,1,'L',1);
				...
				while (list($null,$account) = each($account_info))
				{
						...
						$row_phone = (!$account['account_phone'] ? '-' : $account['account_phone']);
						...
						$pdf->Cell(35,5,$row_phone,0,0,'L',0);
					if ($row_status == lang('Activated'))
					{
						//Restaura cor fonte
						$pdf->SetTextColor(0);
						$pdf->Cell(20,5,$row_status,0,1,'L',0);
					}
					else
					{
						//Muda cor fonte
		...
		function report_users_group()
		{
			...
			$GLOBALS['phpgw_info']['flags']['app_header'] = $GLOBALS['phpgw_info']['apps']['reports']['title'].' - '.lang('report organization');
			$GLOBALS['phpgw']->common->phpgw_header();

			$p = CreateObject('phpgwapi.Template',PHPGW_APP_TPL);
			$p->set_file(array('groups'   => 'report_users_group.tpl'));
		...
		function report_users_group_setor_print()
		{
			$grouplist = trim($_POST[setor]);
			$grouplist = trim(preg_replace("/-/","",$grouplist));
			...
			$p = CreateObject('phpgwapi.Template',PHPGW_APP_TPL);
			$p->set_file(Array('accounts' => 'report_users_group_print.tpl'));
			...
			{ 
				//url do paginador 
				$url = $GLOBALS['phpgw_info']['server']['webserver_url'].'/index.php?menuaction=reports.uireports_users.report_users_group_setor_print';
				...
				$paginas = $this->functions->Paginate_user('accounts',$setordn,$contexts,'cn','asc',$npag,$numpage);
				...
				while (list($null,$accountr) = each($returnp))
				{
					...
					$varr = array(
						...
						'row_phone'		=> (!$accountr['account_phone'] ? '-' : $accountr['account_phone']),
